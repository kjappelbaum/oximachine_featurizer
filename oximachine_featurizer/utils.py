...
def _greedy_farthest_point_samples_non_chunked(
    data, k: int, metric: str = "euclidean", standardize: bool = True
) -> list:
    """
    Args:
        data (np.array)
...
def greedy_farthest_point_samples(
    ...
    if chunked:
        ...
        result = _greedy_farthest_point_samples_non_chunked(
            data, k, metric, standardize
        )

    return result


def _greedy_farthest_point_samples_chunked(
    data, k: int, metric: str = "euclidean", standardize: bool = True
) -> list:
    """
    Args:
...
class SymbolNameDict:
    ...
    def __init__(self):
        with open(
            os.path.join(
                Path(__file__).absolute().parent, "data", "periodic_table.json"
            ),
            "r",
