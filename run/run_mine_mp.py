...
I probably should have used one simple GET request
instead of querying multiple times, but I'll go with it now,
it is not too slow
"""
...
def get_binary_combinations(  # pylint:disable=dangerous-default-value, redefined-outer-name
    metals: list = metals, anions: list = anions
) -> list:
    ...
    for combination in tqdm(combinations):
        ...
        for combination_entry in combination_entries:
            if check_stable(combination_entry.entry_id) and (
                len(combination_entry.as_dict()["composition"]) > 1
            ):
...
def collect_entries():
    ...
    print(
        "*** Starting collect entries for all binary combinations and check if they are stable ***"
    )
